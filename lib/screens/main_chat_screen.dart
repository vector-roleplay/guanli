...
class _MainChatScreenState extends State<MainChatScreen> {
  ...
  void _onScroll() {
    ...
    _scrollButtonTimer = Timer(const Duration(seconds: 1), () {
      if (mounted) {
        setState(() {
  ...
  void _scrollToPreviousMessage() {
    ...
    final totalHeight = _scrollController.position.maxScrollExtent + viewportHeight;
    final avgMessageHeight = totalHeight / messageCount;
    
    final currentMessageIndex = (currentOffset / avgMessageHeight).floor();
    final targetIndex = (currentMessageIndex - 1).clamp(0, messageCount - 1);
    final targetOffset = (targetIndex * avgMessageHeight).clamp(0.0, _scrollController.position.maxScrollExtent);
